...
def _aligned_offset(
    ...
    dim whose own loop really rounded its begin to the sublane and whose block
    size is a multiple of that sublane. Claiming it for an unaligned window would
    let Mosaic read from the wrong row instead of rejecting. Every other offset
    comes back unchanged. Callers with non-block iteration steps must not call
    this helper; ``InnerLoopWindow.steps_by_block`` records that decision.
    """
    sublane = state.device_function.aligned_tiles.get(block_id)
    block = state.device_function.resolved_block_size(block_id)
    if sublane is None or not isinstance(block, int) or block % sublane != 0:
        return offset_expr
    return f"pl.multiple_of({offset_expr}, {sublane})"
            ...
            if bid is not None and bid in block_ids:
                ...
                if not begin_is_zero or (is_store and not covers_full_dim):
                    ...
                    if window.steps_by_block[bid_idx]:
                        start_expr = _aligned_offset(state, bid, start_expr)
...
def _codegen_fori_loop(state: CodegenState) -> object:
    ...
    def _build_dma_slices(
        ...
        *,
        ...
        and advertises the alignment with ``_aligned_offset``.
        """
        from helion._compiler.pallas.ordered_carry import is_dynamic_bound_tile
        ...
        for dim_idx in range(len(shape)):
            ...
            if bid is not None and bid in block_ids:
                ...
                aligned_offset_expr = (
                    _aligned_offset(state, bid, offset_expr)
                    if window.steps_by_block[bid_idx]
                    else offset_expr
                )
